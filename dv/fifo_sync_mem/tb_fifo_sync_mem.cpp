...
void eval(Vfifo_sync_mem &dut) {
    dut.eval();
    ++g_time;
}

void drive_reset(Vfifo_sync_mem &dut, SyncMemScoreboard &ref) {
    dut.clk = 0;
    dut.rst_n = 0;
    dut.push = 0;
    dut.push_data = 0;
...
void tick(Vfifo_sync_mem &dut, SyncMemScoreboard &ref, bool push, uint32_t data, bool pop,
...
void test_external_sync_semantics(Vfifo_sync_mem &dut, SyncMemScoreboard &ref) {
    drive_reset(dut, ref);
    tick(dut, ref, true, 0x10, false, 1, 0);
    tick(dut, ref, true, 0x20, false, 1, 0);
    tick(dut, ref, false, 0, true, 1, 0);
    tick(dut, ref, true, 0x30, true, 1, 0);
    tick(dut, ref, false, 0, true, 1, 0);
    tick(dut, ref, false, 0, true, 1, 0);
    tick(dut, ref, false, 0, true, 1, 0);
...
void test_memory_same_address_and_replacement(Vfifo_sync_mem &dut, SyncMemScoreboard &ref) {
    drive_reset(dut, ref);
    for (uint32_t i = 0; i < kDepth; ++i) {
        tick(dut, ref, true, 0x80 + i, false, kDepth, kDepth - 1);
    }
    tick(dut, ref, true, 0xee, true, kDepth, kDepth - 1);
    tick(dut, ref, true, 0xef, true, kDepth, kDepth - 1);
    for (uint32_t i = 0; i < kDepth; ++i) {
...
void test_overflow_underrun_fallthrough_and_hold(Vfifo_sync_mem &dut, SyncMemScoreboard &ref) {
    drive_reset(dut, ref);
    for (uint32_t i = 0; i < kDepth; ++i) {
        tick(dut, ref, true, 0x40 + i, false);
    }
    tick(dut, ref, true, 0xaa, false);
...
int main(int argc, char **argv) {
    ...
    Vfifo_sync_mem dut;
    SyncMemScoreboard ref;

    test_external_sync_semantics(dut, ref);

module fifo_sync_mem #(
    ...
    initial begin
        ...
        if (DEPTH <= 0) begin
            $fatal(1, "fifo_sync_mem: DEPTH must be greater than 0");
        end
        if ((DEPTH & (DEPTH - 1)) != 0) begin
            $fatal(1, "fifo_sync_mem: DEPTH must be a power of 2");
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            assert ((cfg_almost_full_level_ext >= {{LEVEL_WIDTH{1'b0}}, 1'b1}) &&
                ...
                else $fatal(1, "fifo_sync_mem: cfg_almost_full_level out of range");
            assert (cfg_almost_empty_level_ext <= DEPTH_M1_LEVEL_EXT)
                else $fatal(1, "fifo_sync_mem: cfg_almost_empty_level out of range");

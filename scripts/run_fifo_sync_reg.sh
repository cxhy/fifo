...
run_case() {
  ...
  echo "COVER fifo_sync_reg positive DATA_WIDTH=${dw} DEPTH=${depth} FALL_THROUGH=${ft}"
}

run_illegal_waterline() {
  local case_name="$1"
  local mode_macro="$2"
  local mdir="build/fifo_sync_reg/negative_${case_name}"
  mkdir -p "${mdir}"
  ...
  echo "COVER fifo_sync_reg negative ${case_name}=hit"
}

run_illegal_depth() {
  local mdir="build/fifo_sync_reg/negative_non_power_of_two_depth"
  mkdir -p "${mdir}"
  set +e
  verilator --cc "${rtl}" --top-module fifo_sync_reg --exe dv/fifo_sync_reg/tb_illegal_config.cpp \
    --Mdir "${mdir}" --assert -Wall -Wno-fatal \
  ...
  if [[ "${build_rc}" -ne 0 ]]; then
    negative_count=$((negative_count + 1))
    echo "COVER fifo_sync_reg negative non_power_of_two_depth=elaboration_rejected"
    return
  fi
  if "${mdir}/Vfifo_sync_reg"; then
  ...
  negative_count=$((negative_count + 1))
  echo "COVER fifo_sync_reg negative non_power_of_two_depth=runtime_assertion"
}
...
expected_negative=3
echo "COVERAGE fifo_sync_reg positive_matrix=${positive_count}/${expected_positive} data_widths=${data_widths[*]} depths=${depths[*]} fall_throughs=${fall_throughs[*]} negative_cases=${negative_count}/${expected_negative}"
echo "PASS fifo_sync_reg T008 regression"
